...
class MinecraftPing
{
  ...
  public function Connect()
  {
    ...
    if (!$this->Socket) {
      log_message('debug', "MinecraftPing: Failed to connect or create a socket: $errno ($errstr)");
      return false;
    }
  ...
  public function Query()
  {
    ...
    do {
      if (microtime(true) - $TimeStart > $this->Timeout) {
        log_message('debug', 'MinecraftPing: Server read timed out');
      }
      ...
      if (!$block) {
        log_message('debug', 'MinecraftPing: Server returned too few data');
      }
    ...
    if ($Data === FALSE) {
      log_message('debug', 'MinecraftPing: Server didn\'t return any data');
    }
    ...
    if (JSON_Last_Error() !== JSON_ERROR_NONE) {
      if (Function_Exists('json_last_error_msg')) {
        log_message('debug', 'MinecraftPing: ' . JSON_Last_Error_Msg());
      } else {
        log_message('debug', 'MinecraftPing: JSON parsing failed');
      }
  ...
  private function ReadVarInt()
  {
    ...
    while (true) {
      ...
      if ($j > 5) {
        log_message('debug', 'MinecraftPing: VarInt too big');
      }
